...
@php
    // LOGIKA DINAMIS: Cek ada update admin di 3 hari terakhir (Titik Merah Notif)
  ...
  <div class="bg-white border-r border-[#c3c6d7] flex flex-col justify-between shrink-0 w-[260px] h-screen fixed left-0 top-0 z-50 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
    <div>
      <div class="p-6 flex flex-col gap-1 items-start justify-start relative border-b border-[#e1e2ed]">
        <div class="text-[#004ac6] font-['Manrope-Bold',_sans-serif] text-2xl font-bold tracking-[0.65px]">SCMS</div>
      ...
      <div class="px-3 pt-4 flex flex-col gap-1 relative overflow-y-auto">
        <a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard') ? 'bg-[#d0e1fb] text-[#004ac6]' : 'hover:bg-gray-50 text-[#434655]' }} rounded-lg p-3 flex flex-row gap-3 items-center transition-colors group">
        ...
        <a href="{{ route('complaint.history') }}" class="{{ request()->routeIs('complaint.history', 'complaint.show') ? 'bg-[#d0e1fb] text-[#004ac6]' : 'hover:bg-gray-50 text-[#434655]' }} rounded-lg p-3 flex flex-row gap-3 items-center transition-colors mt-1 group">
        ...
        <a href="{{ route('profile.edit') }}" class="{{ request()->routeIs('profile.edit') ? 'bg-[#d0e1fb] text-[#004ac6]' : 'hover:bg-gray-50 text-[#434655]' }} rounded-lg p-3 flex flex-row gap-3 items-center transition-colors mt-1 group">
  ...
  <div class="ml-[260px] flex flex-col flex-1 relative w-full overflow-hidden min-h-screen">
    ...
    <div class="bg-white/80 backdrop-blur-md px-8 py-4 flex flex-row items-center justify-between z-40 sticky top-0 border-b border-[rgba(195,198,215,0.40)]">
      ...
      <div class="flex flex-row gap-4 items-center">
        <a href="{{ route('complaint.create') }}" class="hidden md:flex bg-[#004ac6] text-white rounded-lg py-2 px-4 items-center gap-2 hover:bg-blue-800 transition shadow-sm">
        ...
        <a href="{{ route('notifications.index') }}" class="rounded-full p-2.5 bg-gray-50 border border-[#c3c6d7] hover:bg-gray-100 transition cursor-pointer relative shadow-sm group">
        ...
        <a href="{{ route('profile.edit') }}" class="flex w-10 h-10 border-2 border-transparent hover:border-[#004ac6] transition-colors rounded-full overflow-hidden shadow-sm">
    ...
    <main class="page-transition w-full flex-1 relative z-10 bg-transparent">
        @yield('content')
    </main>
