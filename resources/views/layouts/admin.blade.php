...
<body class="font-['Manrope',_sans-serif] antialiased bg-[#f8fafc] text-[#191b23] overflow-x-hidden">
@php
...
<div class="flex flex-row items-start min-h-screen w-full relative">
  ...
  <div class="bg-white border-r border-[#c3c6d7] flex flex-col justify-between shrink-0 w-[260px] h-screen fixed left-0 top-0 z-50 shadow-sm">
    <div>
        <div class="p-6 flex flex-col gap-1 items-start justify-start relative border-b border-gray-100">
          <div class="flex items-center gap-2">
              <div class="text-[#004ac6] font-['Manrope-Bold',_sans-serif] text-2xl font-bold">SCMS</div>
              <span class="bg-[#dbe1ff] text-[#004ac6] text-[10px] font-bold px-2 py-0.5 rounded-full">ADMIN</span>
          </div>
          <div class="text-[#434655] font-['Manrope-SemiBold',_sans-serif] text-[13px] font-semibold tracking-[0.65px]">Academic Portal</div>
        </div>
        ...
        <div class="px-3 pt-4 flex flex-col gap-1 relative overflow-y-auto">
          ...
          <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.dashboard') ? 'bg-[#d0e1fb] text-[#004ac6]' : 'hover:bg-gray-50 text-[#434655]' }} rounded-lg p-3 flex flex-row gap-3 items-center transition-colors group">
            <svg class="group-hover:scale-110 transition-transform" width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M8.33333 5V0H15V5H8.33333ZM0 8.33333V0H6.66667V8.33333H0ZM8.33333 15V6.66667H15V15H8.33333ZM0 15V10H6.66667V15H0ZM1.66667 6.66667H5V1.66667H1.66667V6.66667ZM10 13.3333H13.3333V8.33333H10V13.3333ZM10 3.33333H13.3333V1.66667H10V3.33333ZM1.66667 13.3333H5V11.6667H1.66667V13.3333Z" fill="currentColor"/></svg>
  ...
  <div class="ml-[260px] flex flex-col flex-1 relative w-full overflow-hidden min-h-screen">
    ...
    <div class="bg-white/80 backdrop-blur-md px-8 py-4 flex flex-row items-center justify-between z-40 sticky top-0 border-b border-[rgba(195,198,215,0.40)]">
      <div class="text-[#191b23] font-['Manrope-SemiBold',_sans-serif] text-lg font-semibold">
      ...
      <div class="flex flex-row gap-4 items-center">
        ...
        <a href="{{ route('notifications.index') }}" class="rounded-full p-2 hover:bg-gray-100 transition cursor-pointer relative group">
          ...
          @if($pendingCount > 0)
              <div class="bg-[#ba1a1a] rounded-full w-2.5 h-2.5 absolute right-1.5 top-1.5 border-2 border-white"></div>
          @endif
        </a>

        <a href="{{ route('profile.edit') }}" class="flex w-10 h-10 border-2 border-transparent hover:border-[#004ac6] transition-colors rounded-full overflow-hidden">
            <img class="w-full h-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&color=fff&background=004ac6" alt="Admin Avatar"/>
        </a>
      </div>
    </div>
